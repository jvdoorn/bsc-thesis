\documentclass[twoside,single]{lion-msc}

\input{preamble}

\title{Measuring the current-phase relation of a Josephson junction}

\author{J.C.B. van Doorn}
\studentid{s2518074}
\supervisor{M. Rog, MSc \\ \hspace*{\fill}Dr. K. Lahabi}
\corrector{Prof. Dr. J. Aarts}

\abstract{\input{abstract}}

\begin{document}
	\maketitle

	\tableofcontents

	\include{chapters/conventions/main}
	\include{chapters/introduction/main}
	\include{chapters/theory/main}
	\include{chapters/method/main}
	\include{chapters/samples/main}
	\include{chapters/conclusion/main}

	\appendix
	\chapter{Derivations}
	\input{chapters/appendices/derivations}
	\input{chapters/appendices/CP2.6B_data}

	\printbibliography
\end{document}
